...
ROS_INTERFACE ?= "eth0"
ROS_INTERFACE_USE_FIXED_IP ?= "yes"
ROS_INTERFACE_IP ?= "192.168.4.12"

do_configure:append() {
    #Check if the ROS_INTERFACE_USE_FIXED_IP is set to yes
    if [ "${ROS_INTERFACE_USE_FIXED_IP}" = "yes" ]; then
        echo "Setting the ROS_INTERFACE to use the fixed IP"
        echo "ifconfig ${ROS_INTERFACE} ${ROS_INTERFACE_IP}" >> ${S}/network/network_config.sh
    fi
    echo "ifconfig ${ROS_INTERFACE} up" >> ${S}/network/network_config.sh
    #Change NetworkInterfaceAddress in the cyclonedds.xml file
    sed -i "s/NetworkInterfaceAddress=\"eth0\"/NetworkInterfaceAddress=\"${ROS_INTERFACE}\"/g" ${S}/network/cyclonedds.xml
}
